"""Phase 5: Futures Market Validation (ES, NQ)

Test validated factors on equity index futures (ES, NQ).
Goal: Confirm factors generalize across different asset classes.
"""
...
def run_futures_validation_research():
    """Test validated factors on ES and NQ futures."""
    ...
    # Test the best validated factors from crypto research
    factors_config = {
        ...
        "FollowThrough": FactorFilterConfig(follow_through_min=0.6),
        "VolatilityPct": FactorFilterConfig(volatility_pct_min=0.6),
        "TailRatio": FactorFilterConfig(tail_ratio_long_max=-0.2),
        # Best combinations from Phase 4
    ...
    for market, symbol in assets:
        ...
        experiments.append(
            BacktestExperiment(
                name=f"Baseline_{symbol}_IS",
                description=f"EMA 20/50 baseline on {symbol}",
                market=market,
                symbol=symbol,
                timeframe=timeframe,
                ...
                initial_capital=100000.0,  # Larger capital for futures
                factor_filter=None,
            )
        )
        ...
        for factor_name, factor_config in factors_config.items():
            experiments.append(
                BacktestExperiment(
                    name=f"{factor_name}_{symbol}_IS",
                    description=f"{factor_name} on {symbol}",
                    market=market,
                    symbol=symbol,
                    timeframe=timeframe,
                    start_date="2023-01-01",
    ...
    # Run experiments
    print("=" * 100)
    print("PHASE 5: FUTURES MARKET VALIDATION (ES, NQ)")
    print("=" * 100)
    print()
    print(f"Testing top factors across {len(assets)} futures: {', '.join([s for _, s in assets])}")
    print(f"Timeframe: {timeframe}")
    print("In-Sample: 2023 | Out-of-Sample: 2024")
    print("=" * 100)
    print()

    print("[PHASE 5A] In-Sample Testing (2023)")
    print("-" * 100)
    results = {}
    for exp in experiments:
    ...
    print()
    print("[PHASE 5B] Out-of-Sample Validation (2024)")
    print("-" * 100)
    ...
    for market, symbol in assets:
        ...
        baseline_is = results.get(f"Baseline_{symbol}_IS")
        baseline_oos = results.get(f"Baseline_{symbol}_OOS")
        ...
        for factor_name in factors_config.keys():
            is_result = results.get(f"{factor_name}_{symbol}_IS")
            oos_result = results.get(f"{factor_name}_{symbol}_OOS")
    ...
    print()
    print("=" * 100)
    print("CROSS-ASSET SUMMARY (Crypto + Futures)")
    print("=" * 100)
    print()
    ...
    for factor_name in factors_config.keys():
        ...
        for market, symbol in assets:
            ...
            if all([is_result, oos_result, baseline_is, baseline_oos]):
                ...
                print(f"  {status} {symbol}: IS {is_imp:+.2f}%, OOS {oos_imp:+.2f}%")
    ...
    results_data = {
        ...
        "phase": "Phase 5: Futures Market Validation",
        "assets": [s for _, s in assets],
        "timeframe": timeframe,
        "results": {},
    }
    ...
    output_file = output_dir / "futures_validation_phase5.json"
...
if __name__ == "__main__":
    run_futures_validation_research()
