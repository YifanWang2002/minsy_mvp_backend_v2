...
### 2.4 代码整理精简进度（2026-03-06）

- **[已完成] P0** Stripe webhook 成功/失败状态解耦：仅成功写 `processed_at`，失败写 `processing_error + failed_at`，失败可重放。
- **[已完成] P0** Usage 并发幂等与原子累计：增加 `billing_usage_events(user_id, metric_code, reference_type, reference_id)` 唯一约束，事件写入使用 `ON CONFLICT DO NOTHING`，月度聚合使用原子递增。
- **[已完成] P1** API 层移除对 worker task 的直接导入，改为 `apps/api/services/*` + `packages/infra/queue/publishers.py` facade。
- **[已完成] P1** Webhook 500 外部响应收敛为稳定错误码与通用文案，内部异常仅记录日志/监控。
- **[已完成] P2** Deployments 核心业务规则收敛到 domain，route 层保留参数/DTO/异常映射职责。
- **[已完成] P2** Chat quota guard 收敛为单点校验（orchestrator）。
- **[已完成] P2** `billing_return_page` 从路由内联 HTML/CSS 拆分为独立渲染模块。
- **[部分完成] P1/P0-3** 边界治理：本地命令 `uv run python scripts/check_import_boundaries.py` 已通过；**CI 尚未接入该检查**。
- **[已完成] P1** AI usage 持久化失败处理已细化为分类型异常分支（DB/数据/未知），并加入静默失败窗口阈值升级机制；补偿任务与结构化告警持续生效。
- **[已完成] P0-3** route 层“仅依赖 mock service”的专项单测已补齐（无需依赖 worker 实现细节）。
- **[已完成] P3** 死代码清理（删除 `billing.py` 未使用 helper）与 `__pycache__` 噪音目录清理已执行。
...
- `GET /billing/plans`：返回 Free/Go/Plus/Pro 套餐与配额上限
...
- Secrets：`STRIPE_PUBLISHABLE_KEY`、`STRIPE_SECRET_KEY`、`STRIPE_WEBHOOK_SECRET`、`STRIPE_PRICE_GO_MONTHLY`、`STRIPE_PRODUCT_GO`、`STRIPE_PRICE_PLUS_MONTHLY`、`STRIPE_PRICE_PRO_MONTHLY`
