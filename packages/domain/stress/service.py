...
from packages.domain.stress.optimization import (
    build_metric_points,
    build_search_space,
...
from packages.domain.stress.black_swan import run_black_swan_analysis, serialize_window_result
from packages.domain.stress.monte_carlo import run_monte_carlo
from packages.domain.stress.param_sensitivity import scan_param_sensitivity
from packages.domain.stress.scenario_windows import list_windows, resolve_window_set, serialize_windows
...
async def schedule_stress_job(job_id: UUID) -> str:
    ...
    task_id = get_job_queue_ports().enqueue_stress_job(job_id)
