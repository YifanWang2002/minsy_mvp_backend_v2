#!/usr/bin/env python3
"""Verify strategy get/patch MCP calls via ngrok and real OpenAI endpoint."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify strategy get/patch MCP flow with real OpenAI + ngrok"
    )
    ...
    parser.add_argument("--output", default="logs/strategy_patch_mcp_flow_report.json")
    ...
    *,
    ...
    init_result, session_id = _mcp_rpc_call(
        ...
        params={
            ...
            "capabilities": {},
            "clientInfo": {"name": "verify-strategy-patch-mcp-flow", "version": "0.1.0"},
        },
...
async def _prepare_existing_strategy() -> tuple[str, str, int]:
    ...
    async with db_module.AsyncSessionLocal() as db:
        ...
        await db.commit()
        result = (str(session_id), str(created.strategy.id), int(created.strategy.version))
    await db_module.close_postgres()
    return result


async def _create_strategy_session(db: AsyncSession) -> Any:
    email = f"verify_patch_mcp_{uuid4().hex[:10]}@example.com"
...
def main() -> int:
    ...
    try:
        session_id, strategy_id, initial_version = asyncio.run(_prepare_existing_strategy())
            ...
            [
                {
                    "op": "replace",
                    "path": "/strategy/name",
                    "value": "Patched by strategy_patch_dsl",
                }
            ],
            ensure_ascii=False,
        )
        escaped_patch_json = json.dumps(patch_json, ensure_ascii=False)

        client = OpenAI(api_key=api_key)
        cases = [
            Case(
                name="get_existing_strategy",
                tool_name="strategy_get_dsl",
                prompt=(
                    "Call MCP tool strategy_get_dsl exactly once, "
                    f"with session_id='{session_id}' and strategy_id='{strategy_id}'. "
                    "Then output only the tool JSON result."
                ),
                expect_ok=True,
            ),
            Case(
                name="patch_strategy_success",
                tool_name="strategy_patch_dsl",
                prompt=(
                    "Call MCP tool strategy_patch_dsl exactly once, "
                    f"with session_id='{session_id}', strategy_id='{strategy_id}', "
                    f"expected_version={initial_version}, patch_json={escaped_patch_json}. "
                    "Then output only the tool JSON result."
                ),
                expect_ok=True,
            ),
            Case(
                name="patch_strategy_version_conflict",
                tool_name="strategy_patch_dsl",
                prompt=(
                    "Call MCP tool strategy_patch_dsl exactly once, "
                    f"with session_id='{session_id}', strategy_id='{strategy_id}', "
                ...
                expect_ok=False,
                expected_error_code="STRATEGY_VERSION_CONFLICT",
            ),
        ]
        ...
        for index, case in enumerate(cases, start=1):
            print(
                f"[verify-patch-mcp] running case {index}/{len(cases)}: {case.name}",
                file=sys.stderr,
            )
            ...
            print(
                f"[verify-patch-mcp] done case {index}/{len(cases)}: "
