...
- `core._resolve_handler_from_module()` 会从包级 `src.agents.orchestrator.get_handler` 取 handler
...
- `src/agents/orchestrator/strategy_context.py`
...
- `src/agents/orchestrator/fallback.py`
  - `stop_criteria_placeholder.performance_threshold_todo = True` 仍为占位
- `src/agents/skills/stress_test/skills.md` 与 `stages/*.md`
...
- `src/agents/orchestrator/stress_runtime.py`
  - stress phase 的 runtime policy、allowed tools、tool_choice 规则
- `src/agents/orchestrator/stress_postprocess.py`
  - stress 工具结果自动包装（如 `stress_report` GenUI）、状态回填
- `src/agents/orchestrator/deployment_runtime.py`
  - deployment phase tool 暴露策略与最小必需工具集
- `src/agents/orchestrator/deployment_postprocess.py`
...
1. `src/agents/handlers/deployment_handler.py`
...
2. `src/agents/orchestrator/prompt_builder.py`
- 为 deployment 加 runtime policy（工具和 stage）

3. `src/agents/orchestrator/fallback.py`
- 增加 deployment 缺字段场景 fallback 文本

4. `src/agents/skills/deployment/skills.md`
- 增加真实部署 SOP、失败回滚语义、输出契约

### 8.2 建议新增跨层文件（非 orchestrator 包内）

- `src/execution/adapters/paper_adapter.py`
