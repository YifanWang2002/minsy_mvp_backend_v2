...
from src.engine.data.parquet_writer import append_ohlcv_rows
...
from src.engine.market_data.providers.ccxt_rest import CcxtRestProvider
...
def _enqueue_market_data_sync_job(job_id: UUID) -> str:
    from src.workers.market_data_tasks import enqueue_market_data_sync_job

    return enqueue_market_data_sync_job(job_id)


async def schedule_market_data_sync_job(job_id: UUID) -> str:
    """Enqueue one sync job and return Celery task id."""

    task_id = _enqueue_market_data_sync_job(job_id)
