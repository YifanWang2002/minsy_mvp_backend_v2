...
def test_session_history_keeps_non_retryable_validation_failure_even_if_later_success() -> None:
    async def _mock_stream_events(
        *,
        model: str,
        input_text: str,
        ...
        yield {
            "type": "response.output_text.delta",
            "delta": "Validation failed once, then retried.",
            "sequence_number": 1,
        }
        yield {
            "type": "response.output_item.done",
        ...
        yield {
            ...
            "item": {
                ...
                "name": "strategy_validate_dsl",
                "status": "completed",
                "arguments": {"dsl_json": "{\"timeframe\":\"1d\"}"},
        ...
        yield {
            "type": "response.completed",
            "response": {
                "id": "resp_mock_mcp_validation_failure_kept",
    ...
    with patch(
        ...
        with TestClient(app) as client:
            ...
            response = client.post(
                "/api/v1/chat/send-openai-stream",
                headers=headers,
                json={"message": "Run validation twice."},
            )
            assert response.status_code == 200
            payloads = _parse_sse_payloads(response.text)
            done = next(item for item in payloads if item.get("type") == "done")
            ...
            assert len(mcp_calls) == 2
            statuses = [item.get("status") for item in mcp_calls]
