...
def _http_error_code_message(exc: HTTPException | DomainError) -> tuple[str, str]:
    detail = exc.detail
    status_code = getattr(exc, "status_code", 500)
    if isinstance(detail, dict):
        code_raw = detail.get("code")
        message_raw = detail.get("message")
        code = str(code_raw).strip() if code_raw else f"HTTP_{status_code}"
        message = (
            str(message_raw).strip()
            if message_raw
            else str(detail).strip() or f"HTTP {status_code}"
        )
        return code, message
    text = str(detail).strip()
    return f"HTTP_{status_code}", text or f"HTTP {status_code}"
...
async def trading_create_paper_deployment(
    ...
    try:
        async with await _new_db_session() as db:
            ...
            created = await deployment_ops.create_deployment(
                db=db,
            ...
            deployment_payload = deployment_ops.serialize_deployment(created)

            queued_task_id: str | None = None
            started = False
            if bool(auto_start):
                loaded_deployment = await deployment_ops.load_owned_deployment(
                    db,
                    deployment_id=created.id,
                    user_id=claims.user_id,
                )
                started_deployment = await deployment_ops.apply_status_transition(
                    db,
                    deployment=loaded_deployment,
                    target_status="active",
                )
                deployment_payload = deployment_ops.serialize_deployment(started_deployment)
...
async def trading_list_deployments(
    ...
    try:
        async with await _new_db_session() as db:
            ...
            deployments = [
                _serialize_model(deployment_ops.serialize_deployment(row))
                for row in rows
            ]
    except (HTTPException, DomainError) as exc:
        code, message = _http_error_code_message(exc)
...
async def _transition_deployment(
    *,
    ...
    try:
        async with await _new_db_session() as db:
            deployment = await deployment_ops.load_owned_deployment(
                db,
                deployment_id=deployment_uuid,
                user_id=user_id,
            )
            updated = await deployment_ops.apply_status_transition(
                db,
            ...
            deployment_payload = _serialize_model(
                deployment_ops.serialize_deployment(updated)
            )
    except (HTTPException, DomainError) as exc:
        code, message = _http_error_code_message(exc)
        return _payload(
...
async def trading_get_positions(
    ...
    try:
        async with await _new_db_session() as db:
            await deployment_ops.load_owned_deployment(
                db,
                deployment_id=deployment_uuid,
                user_id=user_id,
            )
            ...
            positions = [
                _serialize_model(deployment_ops.serialize_position(position))
...
async def trading_get_orders(
    ...
    try:
        async with await _new_db_session() as db:
            ...
            orders = [
                _serialize_model(deployment_ops.serialize_order(order))
                for order in rows
            ]
    except (HTTPException, DomainError) as exc:
        code, message = _http_error_code_message(exc)
