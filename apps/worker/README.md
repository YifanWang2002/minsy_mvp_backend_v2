...
| P1（收尾） | 队列隔离与成本守护（含 Redis 拆库演进） | `dev` 已完成 Phase1（`REDIS_DB=0`, `CELERY_BROKER_URL=/1`, `CELERY_RESULT_BACKEND=/2`）；`prod` 仍共用 `/0`，待对齐 | 高（稳定性） | 中 | 收尾顺序：先对齐 prod DB 分离，再加 Redis 维度监控与告警，最后评估是否升级为独立实例隔离 |
